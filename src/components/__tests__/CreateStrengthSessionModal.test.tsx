...
describe("CreateStrengthSessionModal", () => {
  ...
  test("moves search to editor to an ordered draft and quick logs", async () => {
    ...
    await fireEvent.press(view.getByText("Quick Log"));
  ...
  test("saves a reusable template without closing and updates the same template", async () => {
    ...
    expect(view.getByText("Workout saved.")).toBeTruthy(); expect(onClose).not.toHaveBeenCalled(); expect(mockCreateTemplate).not.toHaveBeenCalled();
  });
