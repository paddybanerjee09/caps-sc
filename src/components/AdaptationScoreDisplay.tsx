...
type ScoreResult<K extends string> =
  | { status: "insufficient"; reasons: string[] }
  | { status: string; scores: Record<K, number>; evidence: "full" | "limited"; missingInputs: string[] };
export function AdaptationScoreDisplay<K extends string>({ onBack, result, order, presentations, disclaimer, backLabel, fullEvidenceText, limitedEvidenceText }: {
  onBack: () => void; result: ScoreResult<K>; order: readonly K[];
  presentations: Record<K, { label: string; color: string }>;
  disclaimer: string; backLabel: string; fullEvidenceText: string; limitedEvidenceText: string;
}) {
